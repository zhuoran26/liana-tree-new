...
## Temperature and relative humidity data are for the top of the canopy (i.e. tower extending higher than the top of the canopy)
## I chose top of canopy because we are assuming one layer of leaves, such that all leaves would be exposed to the upper most part of the canopy
...
bci_temp = subset(bci_temp, bci_temp$chk_note != 'bad' & bci_temp$chk_note != 'doubtful' & bci_temp$chk_note != 'nc' & bci_temp$chk_note != 'missing')
bci_rh = subset(bci_rh, bci_rh$chk_note != 'bad' & bci_rh$chk_note != 'doubtful' & bci_rh$chk_note != 'nc' & bci_rh$chk_note != 'missing')
...
bci_temp_rh = subset(bci_temp_rh, !is.na(bci_temp_rh$at) & !is.na(bci_temp_rh$rh))
...
# Find mean VPD in frormat month x hour
for(i in 1:12){
  for(j in 0:23){
    bci_vpd_matrix[i,j+1] = mean(bci_temp_rh$VPD_Pa[which(as.numeric(substr(bci_temp_rh$dt, 6, 7)) == i & as.numeric(substr(bci_temp_rh$dt, 12, 13)) == j)], na.rm = T)
...
# Change column names
colnames(data_swp) = c('Month', 'D_550_cm', 'D_450_cm', 'D_350_cm', 'D_250_cm', 'D_175_cm', 'D_125_cm', 'D_90_cm', 'D_70_cm', 'D_50_cm', 'D_35_cm', 'D_25_cm', 'D_15_cm', 'D_7.5_cm', 'D_2.5_cm')
