...
#changing the canopy by using tree/liana's frac.al
...
for(k in 1:length(varyk)){
  for(j in 1:12){
    ...
    K = varyk[k]
    #here is where frac.tree.al is used
...
#after loading, this gives another set of vpd and SWP
...
#sap area is determined by dbh, why min
for(i in 1:nrow(df1)){
...
for(i in 1:nrow(df2)){
  ## Functional xylem cross-sectional area, from Reyes-García et al. 2012
