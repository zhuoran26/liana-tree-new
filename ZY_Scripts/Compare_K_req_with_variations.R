#compare k req between month and hour using VPD variations
rm(list = ls())
library(dplyr)
library(ggplot2)
library(ggsci)
...
colnames(liana_out_mon_1) <-c("NPP_sum","K")
liana_out_mon_1$K_req <- liana_out_mon_1$K/1000
liana_out_mon_1$dbh<-dbhs
...
liana.NPP.mxh.hour = function(dbh, psis, D_vec, liana.k, SLA = NULL, b1 = NULL, 
  ...
  #NPP_no_leaves = (1 - rG) * (A1_h  * 0 - rd_h  * 0 - rr_h  - rx_turn  - rp_turn ) * 1e-9 * 12
  return(NPP)
}

lianaouteach.hour = rep(0,8784)

for (j in 1:length(year_day_light)){
  psis = year_SWP[j]
  D = year_vpd[j]
  dbh = dbhs[1]
  K = 153155
  day_light=year_day_light[j]
  lianaouteach.hour[j] = liana.NPP.mxh.hour(dbh = dbh, liana.k = K, tot.al = tot.al,
                                            frac.liana.al = frac.liana.al, liana.length = liana.length,
                                            day_light=day_light,psis = psis,D_vec=D)
}
sum(lianaouteach.hour)
