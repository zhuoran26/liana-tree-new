#this code was not used for anything
#merge monthly and daily to 1 function
...
###only Jan, so we change nday to either 1 or 31, need to think more on yearly NPP
###D_vec here is the new VPD!use old vpd to check if function works (checked in the old code)
###got stuck:
###don't know how to modify the monthly vs daily A and R, thought about doing mini loops, see line 166
###also what do I do with putting VPD, when it is monthly vs daily
liana.NPP.mxh.new.jan = function(dbh, psis, D_vec, liana.k, SLA = NULL, b1 = NULL, 
                             b2 = NULL, nday,tot.al, frac.liana.al, 
  ...
  stem.turn = 0.1 / 365 *nday #nday[month] #yearly turnover / months
  ...
  #if nday >1, rx_turn = rx_d*nday - rx_lost*nday
  ...
  if(NPP < 0){
    stem.turn = 0.162 /365*nday #made change
