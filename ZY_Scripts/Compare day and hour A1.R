#trying to see how NPP varies hourly, make it hourly
rm(list = ls())
...
july_met <- subset(vpd_year,month==7)
july_vpd <- july_met$vpd_pa
#get just one day
first_day_vpd <-july_vpd[0:24]
first_day_light <- july_day_light[0:24]
K=5805.55556
...
lianaouteach.day = rep(0,1)

    psis = SWP[7]
    D_vec = first_day_vpd
    
    liana.out = liana.NPP.mxh.day(dbh = dbh, psis = psis,  D_vec = first_day_vpd,liana.k = K, tot.al = tot.al, nday=nday,month=month,
...
#now test hourly
july_org_vpd = VPD[7,]
july_org_hourly_VPD= rep(july_org_vpd,31)
...
first_day_A1 = rep(0,24)

for (i in 1:24){
  
  first_day_A1[i] = liana.NPP.mxh.hour(dbh = dbh, liana.k = K, tot.al = tot.al, nday=nday,month=month,
