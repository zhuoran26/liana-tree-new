#this script was used to check the original code, not output
liana.NPP.mxh.mon = function(dbh, psis, D_vec, liana.k, SLA = NULL, b1 = NULL, 
  ...
  D = movavg(VPD[1,], 2, type = 's') # Smoothed, hourly VPD
  ...
  for(hour in 1:24){
    ...
    Kmax = K # Hydraulic conductivity, from meta-analysis
    ...
    phi_s = (Kmax * log(exp(b1 * SWP[1] - b1 * b2) + 1)) / b1 # Integral of conductivity (mmol/m/s/MPa), default equation
