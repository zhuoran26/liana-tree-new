...
tree.NPP.mxh = function(tree.dbh, psis, D_vec, tree.k, SLA = NULL, b1 = NULL, 
  ...
  # Montly Anet
  Anet = (1 - rG) * (A1_m * tree.al - rd_m * (tree.al) - rr_m - rx_m - rp_m) * 1e-9 * 12
  
  # Monthly turnover & NPP#change it to daily /nday
  ...
  NPP_prelim = (1 - rG) * (A1_m * tree.al - rd_m * (tree.al) - rr_m - rx_turn - rp_turn) * 1e-9 * 12
  
  # Allows leaves to drop if water stress is present
  if(NPP_prelim < 0){
    tree.al = 0
    NPP = ((1 - rG) * (A1_m * tree.al - rd_m * (tree.al) - rr_m - rx_turn - rp_turn) * 1e-9 * 12)
  }else{
    NPP = NPP_prelim
  }
...
#add in hourly
## Liana model
liana.NPP.mxh.org = function(dbh, psis, D_vec, liana.k, SLA = NULL, b1 = NULL, 
...
lianaouteach.org = rep(0,12)
#liana_out_r1 = matrix(, nrow = length(dbhs), ncol = 2)

    for(j in 1:12){
      nday = c(31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31)#how many days in each month
      month = j
      ...
      K = K
      liana.out = liana.NPP.mxh.org(dbh = dbh, psis = psis, D_vec = D, liana.k = K, nday = nday, month = month, tot.al = tot.al, frac.liana.al = frac.liana.al, liana.length = liana.length)
      lianaouteach.org[j] = liana.out[1]
    }
