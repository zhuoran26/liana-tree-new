...
#monthly NPP function (changed rG only)
...
for(j in 1:1){
  ...
  liana.out = liana.NPP.mxh.mon(dbh = dbh, psis = psis, D_vec = D, liana.k = K, 
                                nday = nday, month = month, tot.al = tot.al, 
                                frac.liana.al = frac.liana.al, liana.length = liana.length)
  lianaouteach.mon.jan[j] = liana.out[1]
}
lianaouteach.mon.jan

#run monthly and daily loop
lianaouteach.day.jan = rep(0,1)
...
for(j in 1:1){
  ...
  for(i in 1:31){
    ...
    liana.out = liana.NPP.mxh.day(dbh = dbh, psis = psis, D_vec = D, 
                                  liana.k = K, tot.al = tot.al,
                                  nday=nday,month=month,
                                  frac.liana.al = frac.liana.al, 
                                  liana.length = liana.length)
    lianaouteach.day.jan[j] = lianaouteach.day.jan[j]+liana.out[1]
  }
}

lianaouteach.mon.jan
lianaouteach.day.jan
#checked with new K (4/12)
