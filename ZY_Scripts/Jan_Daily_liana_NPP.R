## Liana model, only Jan
liana.NPP.mxh.jan = function(dbh, psis, D_vec, liana.k, SLA = NULL, b1 = NULL, 
                         b2 = NULL, tot.al, frac.liana.al, #made changes, no nday
  ...
  # Monthly turnover & NPP changed to Jan turnover
  stem.turn = 0.1 / 12/ 31 #nday[month] #yearly turnover / months
  ...
  if(NPP < 0){
    stem.turn = 0.162 /12 /31 #made change
...
#test
#D = movavg(VPD[1,], 25, type = 's') 25, which is greater than 24 would not run, but 2 would

liana.NPP.mxh.jan (dbh_test, SWP[1], VPD[1,], SLA = NULL, b1 = NULL, 
                                 b2 = NULL, liana.k = k_test, tot.al=2 * 100, frac.liana.al=0.1, 
                   liana.length=(tree.b1Ht * tree.dbh^tree.b2Ht), Vm = NULL)
#D_avg got stuck

#using the function to compute Jan only value(Error in movavg(D_vec, 2, type = "s") : 
#Window length 'n' must be greater then length of time series.
#Called from: movavg(D_vec, 2, type = "s"))
...
#choose 1 k and 1 dbh

k_test=varyk[1]
dbh_test = dbhs[1]

lianaouteach = c() #should give me 24 values
...
for(j in 1){
  ...
  frac.liana.al = 0.1
      ...
      D = VPD[j,]#average diurnal cycle, but I read in this 31 times for each loop
      
      if(j==1){
        liana.length = (tree.b1Ht * tree.dbh^tree.b2Ht)#why tree
      }else{
        liana.length = liana.out[4]
      }
      K = k_test
      liana.out = liana.NPP.mxh.jan(dbh = dbh, psis = psis, 
                                    D_vec = D, liana.k = K,
                                    tot.al = tot.al, frac.liana.al = frac.liana.al, 
                                    liana.length = liana.length)
      #usage:liana.NPP.mxh.jan = function(dbh, psis, D_vec, liana.k, SLA = NULL, b1 = NULL, 
      #b2 = NULL, tot.al, frac.liana.al, #made changes, no nday, nmonth
      #liana.length, Vm = NULL){
      lianaouteach = liana.out[1] #return(c(NPP, Lx, tree.al, stem.turn, Lx_turn, A1))
    }
