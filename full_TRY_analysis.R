## This script takes global datasets on individual functional traits from the TRY database
## Then processes the data, including a coarse filtering for tropical trees and lianas
## and ensures data are in consistent units
## In the section headers, the name of the plant functional type is given
## along with the download identifier we received from TRY
...
# Storage: species identifiers x 2 matrix
output = matrix(, nrow=max(spp$AccSpeciesID), ncol=2)

count = 0

# Loop through species identifiers
for(i in uniquespecies){
  # Species ID
  sp = subset(spp, AccSpeciesID == i)
  # PFT assignments corresponding to species ID
  pfts = sp$OrigValueStr
  # If the species is assigned tree or liana PFT, 
  # use whichever one is used more frequently
  if(length(grep('tree', pfts, ignore.case = T)) > 0 | length(grep('liana', pfts, ignore.case = T)) > 0){
    ...
    # Same here, but for common abbreviations corresponding to liana and tree
  }else{
    if('T' %in% pfts | 'TS' %in% pfts | 'L' %in% pfts){
      ...
      # Assign everything else to another category
    }else{
      type = 'non-woody'
    }
  }
...
# Force non-numeric entries to numeric values
for(i in 1:length(units$DatasetID)){
  if(units$OrigValueStr[i] %in% c('<=1yr','>=1yr')){ # basic estimate
    units$OrigValueStr[i] = 1
  }else{
    if(units$OrigValueStr[i] == '>24'){ # basic estimate
...
for(i in 1:length(sp.sla$OrigUnitStr)){
  if(sp.sla$OrigUnitStr[i] %in% c('mm2/mg', 'mm2 / mg',
                                  'mm2 mg-1', 'mm2*mg-1',
                                  'mm^2/mg', 'mm\xb2/mg')){
...
for(i in 1:length(sp.n$DatasetID)){
  if(sp.n$OrigUnitStr[i] %in% c('mmol/m2')){
    ...
    if(sp.n$OrigUnitStr[i] %in% c('g / m2', 'g N m-2',
                                  'g/m2', 'g m-2', 'g m2',
                                  'gN m-2', 'gN.m-2')){
...
# Make the units consistent
# I made the decisions to assume that \xb5 refers to micro, 
# and that m-1 should have been m-2
for(i in 1:length(sp.a$DatasetID)){
  if(sp.a$OrigUnitStr[i] %in% c('micro mol/m2/s', 'micromol/m2/s',
                                'micromoles/m2/s', 'umol CO2 m-2 s-1',
                                'umol/m2/s', 'micro mol m-2 s-1',
                                'micromol m-2 s-1', 'umol CO2/m2/s',
                                'umolCO2/m2-s', '\xb5mol(CO2) m-2 s-1',
                                'micro mol CO2 m-2 s-1',
                                'umol CO2 / m2 / sec', 'umol CO2/m^2 s',
                                'micromolco2 m-2 s-1', 'micromol CO2 m-2 s-1',
                                'micromol CO2 m-1s-1', 'micromol. m-2. s-1', 'micro mol C m-2 s-1')){
...
for(i in 1:length(sp.ssd$DatasetID)){
  if(sp.ssd$OrigUnitStr[i] %in% c('kg/m3', 'kg m-3')){
    ...
    if(sp.ssd$OrigUnitStr[i] %in% c('g/dm3')){
      ...
      if(sp.ssd$OrigUnitStr[i] %in% c('g / cm3', 'g/cm^3', 'g/cm3',
                                      'g_cm3-1', 'g cm-3', 'g*cm-3', 'g /(cm3*10)')){
...
# Add column names to output
colnames=c('Species ID', 'Growth form', 'Leaf lifespan (m)',
           'SLA (mm2/mg)', 'Narea (g/m2)', 'Aarea (mmol CO2/m2/s', 'SSD (g/cm3)')
colnames(output) = colnames
...
sp.srl = subset(sp.srl, select = c('DatasetID', 'AccSpeciesID', 'ObservationID',
                                   'TraitID', 'OrigValueStr', 'OrigUnitStr', 'OriglName'))
# Take out all the rows that do not correspond with root length or growth type
...
colnames=c('Species ID', 'Growth form', 'Leaf lifespan (m)',
           'SLA (mm2/mg)', 'Narea (g/m2)', 'Aarea (mmol CO2/m2/s',
           'SSD (g/cm3)', 'SSC (kg/m/s/MPa)', 'SRL (m/g)')
colnames(output) = colnames
...
# Convert all observations into a single value. 
# Ranges are condensed to median value
...
colnames=c('Species ID', 'Growth form', 'Leaf lifespan (m)',
           'SLA (mm2/mg)', 'Narea (g/m2)', 'Aarea (mmol CO2/m2/s',
           'SSD (g/cm3)', 'SSC (kg/m/s/MPa)', 'SRL (m/g)', 'FRD (mm)')
colnames(output) = colnames
...
colnames=c('Species ID', 'Growth form', 'Leaf lifespan (m)',
           'SLA (mm2/mg)', 'Narea (g/m2)', 'Aarea (mmol CO2/m2/s',
           'SSD (g/cm3)', 'SSC (kg/m/s/MPa)', 'SRL (m/g)', 'FRD (mm)', 'CRD (mm)')
colnames(output) = colnames
...
sp.mc = subset(sp.mc, select = c('DatasetID', 'AccSpeciesID', 'ObservationID',
                                 'TraitID', 'OrigValueStr', 'OrigUnitStr', 'OriglName'))
# Take out all the rows that do not correspond with mycorrhizal colonization or growth type
...
colnames = c('Species ID', 'Growth form', 'Leaf lifespan (m)',
             'SLA (mm2/mg)', 'Narea (g/m2)', 'Aarea (mmol CO2/m2/s',
             'SSD (g/cm3)', 'SSC (kg/m/s/MPa)', 'SRL (m/g)', 'FRD (mm)',
             'CRD (mm)', 'MC (%)', 'RHC (cm3/cm/s/MPa*10^6)')
colnames(output) = colnames
...
colnames = c('Species ID', 'Growth form', 'Leaf lifespan (m)',
             'SLA (mm2/mg)', 'Narea (g/m2)', 'Aarea (mmol CO2/m2/s',
             'SSD (g/cm3)', 'SSC (kg/m/s/MPa)', 'SRL (m/g)','FRD (mm)',
             'CRD (mm)', 'MC (%)', 'RHC (cm3/cm/s/MPa*10^6)', 'WVD (um)')
colnames(output) = colnames
...
colnames = c('Species ID', 'Growth form', 'Leaf lifespan (m)',
             ...
             'SSD (g/cm3)', 'SSC (kg/m/s/MPa)', 'SRL (m/g)','FRD (mm)',
             'CRD (mm)', 'MC (%)', 'RHC (cm3/cm/s/MPa*10^6)', 'WVD (um)',
             'Dens (1/mm^2)', 'Height (m)', 'LA (cm^2)', 'SM (mg)')
colnames(output) = colnames
...
colnames = c('Species ID', 'Growth form', 'Leaf lifespan (m)',
             ...
             'SSD (g/cm3)', 'SSC (kg/m/s/MPa)', 'SRL (m/g)', 'FRD (mm)',
             'CRD (mm)', 'MC (%)', 'RHC (cm3/cm/s/MPa*10^6)', 'WVD (um)',
             'Dens (1/mm^2)', 'Height (m)', 'LA (cm^2)', 'SM (mg)', 'WVEL (um)')
colnames(output) = colnames
...
for(i in 1:length(sp.nmass$DatasetID)){
  if(sp.nmass$OrigUnitStr[i] %in% c('mg/g', 'mg g-1', 'mg_g-1',
                                    '\xb5g mg-1', 'g/kg','mg/g dry mass',
                                    'mg / g', 'mg N g-1', 'g kg-1')){
...
colnames = c('Species ID', 'Growth form', 'Leaf lifespan (m)',
             ...
             'CRD (mm)', 'MC (%)', 'RHC (cm3/cm/s/MPa*10^6)', 'WVD (um)',
             'Dens (1/mm^2)', 'Height (m)', 'LA (cm^2)', 'SM (mg)',
             'WVEL (um)', 'RRD (m)', 'P50 (MPa)', 'Nmass (mg/g)')
colnames(output) = colnames
...
colnames = c('Species ID', 'Growth form', 'Leaf lifespan (m)',
             ...
             'CRD (mm)', 'MC (%)', 'RHC (cm3/cm/s/MPa*10^6)', 'WVD (um)',
             'Dens (1/mm^2)', 'Height (m)', 'LA (cm^2)', 'SM (mg)',
             'WVEL (um)', 'RRD (m)', 'P50 (MPa)', 'Nmass (mg/g)', 'Pmass (mg/g)')
colnames(output) = colnames
...
## This output file is further processed manually as described in the Methods
## of the manuscript associated with this code